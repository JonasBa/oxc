...
impl<'a> PeepholeOptimizations {
    ...
    fn is_tdz_closed_over_read(symbol_id: SymbolId, ctx: &TraverseCtx<'a>) -> bool {
        ...
        // before any function/arrow boundary means the binding is local to the
        // current function (safe). Crossing a function boundary first means it
        // is closed over (potential TDZ hazard).
        for scope in ctx.scoping().scope_ancestors(ctx.current_scope_id()) {
            if scope == decl_scope {
                return false;
            }
            if ctx.scoping().scope_flags(scope).intersects(ScopeFlags::Function | ScopeFlags::Arrow)
            {
                return true;
            }
        }
